...
public class Spiel {
	...
	private static void setupSpiel(Spieler spieler) {
		...
		createSpiel();
		// TODO Mouseinput implementieren
		spiel = new Spiel();
		//test = new Test(spiel, spieler);
		GUI_Spielfeld test = new GUI_Spielfeld(spieler);
	...
	private static void initSpielfeldStatus() {
		...
		// Spielfeldstatus ist um 2 Einheiten größer als das angezeigte Spielfeld
	...
	private static void setSpielfeldMinen() {
		...
		for (int i=0; i<anzahlMinen; i++) {
			...
			//debug: JOptionPane.showMessageDialog(null, "x: " + x + "/ y: " + y);
		}
	}
	...
	private static void setSpielfeldZahlen() {
		...
		// Es durchläuft den äußeren Rand nicht, da es nicht zum angezeigten Spielfeld gehört
	...
	// TODO muss an neuen Ort (spiel) angepasst werden. enthält noch elemente, die eventuell in GUI_Spielfeld geschehen.
	// Zeitmessung könnte auch komplett in Spiel stattfinden
